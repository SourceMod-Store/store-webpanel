        ...
        if ($this->upload->do_upload('importFile'))
        {
            ...
            foreach ($json->categories as $category)
            {
                ...
                foreach ($category->items as $item)
                {
                    ...
                    {
                        $this->items_model->add_item($item->name, $item->display_name, $item->description, $item->web_description, $item->type, $item->loadout_slot, $item->price, json_encode($item->attrs, JSON_UNESCAPED_SLASHES), $item->is_buyable, $item->is_tradeable, $item->is_refundable, $category_id, $item->expiry_time,$item->flags);
                    }
                    else
                    {
                        $this->items_model->add_item($item->name, $item->display_name, $item->description, $item->web_description, $item->type, $item->loadout_slot, $item->price, json_encode($item->attrs), $item->is_buyable, $item->is_tradeable, $item->is_refundable, $category_id, $item->expiry_time,$item->flags);
                    }
                }
            }
